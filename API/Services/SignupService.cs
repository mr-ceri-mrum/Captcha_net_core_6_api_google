...
namespace API.Services
{
    public class SignupService : ISignupService
    {
        ...
        public async Task<SignupResponse> Signup(SignupRequest signupRequest)
        {
            ...
            {
                { "secret", appSettings.RecaptchaSecretKey }, { "response", signupRequest.ReCaptchaToken }
            };
            ...
            {
                recaptchaResponse =
                    await http.PostAsync("https://www.google.com/recaptcha/api/siteverify", postContent);
            ...
            if (!recaptchaResponse.IsSuccessStatusCode)
            {
                ...
                {
                    Success = false, Error = "Unable to verify recaptcha token", ErrorCode = "S03"
                };
            }
            ...
            if (string.IsNullOrEmpty(stringContent))
            {
                return new SignupResponse()
                {
                    Success = false, Error = "Invalid reCAPTCHA verification response", ErrorCode = "S04"
                };
            }
            ...
            if (googleReCaptchaResponse.Score < 0.5)
            {
                return new SignupResponse()
                {
                    Success = false, Error = "This is a potential bot. Signup request rejected", ErrorCode = "S07"
                };
            }
